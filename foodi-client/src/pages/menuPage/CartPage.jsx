...
import { useTheme } from "../../hooks/ThemeContext"; // Assuming you have this hook for dark mode
...
const CartPage = () => {
  ...
  const handleProceedToCheckout = () => {
    Swal.fire({
      ...
      customClass: {
        popup: `${isDarkMode ? 'bg-gray-900 text-white' : 'bg-white text-black'} p-3 rounded-lg shadow-lg max-w-xs`, 
      ...
      width: '280px', // Slightly adjusted width for better responsiveness
      padding: '0.75rem', // Further reduced padding for a more compact view
    }).then((result) => {
      if (result.isConfirmed) {
  ...
  return (
    <div className={`section-container px-4 py-8 sm:px-6 lg:px-8 ${isDarkMode ? 'bg-black text-white mt-20' : 'bg-white text-black'}`}>
      
      {/* Title and Video in Flexbox */}
      <div className={`flex items-center justify-center gap-4  ${isDarkMode? "sm:mb-10 md:mb-10 lg:mb-10":"sm:mb-0 md:mb-0 lg:mb-0"}`}>
        {/* Video Section */}
        {!isDarkMode && (
          <video
          ...
          <div>
            <div className="overflow-x-auto">
              <table className="table w-full min-w-full">
                ...
                <tbody>
                  {cart.map((item, index) => (
                    <tr key={index} className={isDarkMode ? 'bg-black text-white  ' : 'bg-white text-black'}>
                      <td>{index + 1}</td>
                      <td>
                      ...
                      <td className="flex items-center">
                        ...
                        <input
                          ...
                          className={`w-10 mx-2 text-center ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-white text-black'}`}
                        />
                        <button
                          className="btn btn-xs"
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center my-8 gap-6">
              ...
              <div className="md:w-1/2 space-y-2">
                ...
                <button className="btn btn-md bg-green text-white px-6 py-2 mt-2" onClick={handleProceedToCheckout}>
                  Proceed to Checkout
                </button>
              </div>
            </div>
