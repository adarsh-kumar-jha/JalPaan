  ...
  return (
    <div className="w-full sm:w-[400px] border rounded-lg overflow-hidden shadow-lg dark:bg-gray-800 dark:border-gray-700">
      <div className="px-4 sm:px-6 py-4">
        <div className="font-bold text-lg sm:text-xl mb-2 text-green dark:text-green-300">Learn To Cook</div>
        <div className="mb-4">
          <label
    ...
    <div>
      {paragraphs.map((paragraph, index) => {
        ...
        if (index === 0 && !isHeading) {
          // Assuming the first paragraph is the dish name
          return (
            <motion.div
              key={index}
...
function Ai() {
  ...
  async function onSubmit(data) {
    setLoading(true);
    try {
      const response = await axios.post("http://localhost:6001/recipeStream/chat", data);
        ...
        <div className="w-full sm:w-[400px] h-[400px] sm:h-[565px] text-xs text-gray-600 dark:text-gray-300 p-4 border rounded-lg shadow-xl dark:border-gray-700 whitespace-pre-line overflow-y-auto mt-4 sm:mt-0">
          {loading ? (
            <div className="flex justify-center items-center h-full">
              ...
              <span className="ml-4 dark:text-white">Please Wait, Generating Recipe...</span>
            </div>
          ) : (
